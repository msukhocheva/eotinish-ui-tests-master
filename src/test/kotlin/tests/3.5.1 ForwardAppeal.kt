...
class ForwardAppeal (private var assert: Boolean = true, private var appealType: String = listOfAppealTypes[0]) {
    ...
    fun forwardAppealTest() {
        ...
        // переадресовываем подготавливаем к частичной переадресации все созданные обращениям и отправляем их на согласование и подписание
        ...
        // проверяем наличие переадресованных обращений, а также сроки исполнения в карточке обращения
        ...
        for (i in regNumbersOfCreatedAppeals.indices) {
            ...
            //Проверяем срок исполнения в карточке обращения
            if (assert) {
        ...
        if (assert) {
            println("Test 3.5.1   passed (${typesOfCreatedAppeals[0]}, ЖТ-2022-${regNumbersOfCreatedAppeals[0]}, ЖТ-2022-${regNumbersOfCreatedAppeals[0]}/1)")
