...
//Исполнение жалоб без заслушивания

class ExecuteComplaintsWithoutHearing {
    ...
    @Test
    fun executeComplaintsWithoutHearing() {
        for (i in listOfComplaintTypes.indices) {
            ...
            //В поле поиска вставляем номер жалобы, ищем и открываем ее
            ...
            typesOfCreatedComplaints.add(complaintTypeName)
            regNumbersOfCreatedComplaints.add(complaintNumber)
        }

        //Логинимся под Исполнителем, принимаем решения по всем созданным жалобам и отправляем на согласование и подписание
        //Проверяем залогинен ли Исполнитель, если нет, то логинимся под ним
        switchOnUser(org1Executor1)

        //Проходимся по каждой полученной в работу жалобе
        for (i in regNumbersOfCreatedComplaints.indices) {
            ...
            org1Executor1.page.fill(inWorkAppealsListPage.appealsSearch_input, regNumbersOfCreatedComplaints[i].substring(3))
            ...
            //Проверяем корректно ли отображаются типы решений, типы причин и оснований для вынесения решения
            if (assert) {
        ...
        //Логинимся под Подписантом, согласовываем и подписываем все полученные на согласование и подписание жалобы
        ...
        for (i in regNumbersOfCreatedComplaints.indices) {
            ...
            org1Chief1.page.fill(inWorkAppealsListPage.appealsSearch_input, regNumbersOfCreatedComplaints[i].substring(3))
        ...
        //Проверям находится ли исполненная жалоба во вкладке "Завершенные" и отображается ли статус "Исполнено" в статусах обращения
        if (assert) {
            for (i in regNumbersOfCreatedComplaints.indices){
                org1Chief1.page.click(asideToggle.asideToggleFix_btn)
            ...
            println("Test 3.4.2   passed (${typesOfCreatedComplaints[0]}, ЖТ-2022-${regNumbersOfCreatedComplaints[0]})")
