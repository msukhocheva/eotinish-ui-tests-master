...
class ReassignCoExecutor(private var assert: Boolean = true) {
    ...
    fun reassignCoExecutor() {
        ...
        //Проверяем залогинен ли Руководитель, если нет, то логинимся под ним и направляем в работу Исполнителю-1 и Исполнителю-3(Соисполнителю-2)
        switchOnUser(org1Chief1)
        ...
        //Проверяем залогинен ли Исполнитель-3(Соисполнитель-2), если нет, то логинимся под ним и проверяем есть ли обращение
        ...
        if (assert) {
            ...
            println("Test 3.2.6   passed ($appealTypeName ЖТ-2022-$appealNumber)")
        }
    }
}
