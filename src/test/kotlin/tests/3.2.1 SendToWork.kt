...
class SendToWorkByRegistrarToChief(private var assert: Boolean = true) {
    ...
    fun sendToWorkTest() {
        ...
        if (assert) {
            assertEquals(org1Chief1.page.innerText(appealCardPage.cardHead_text), "$appealTypeName №ЖТ-2022-$appealNumber")
            println("Test 3.2.1   passed ($$appealTypeName ЖТ-2022-$appealNumber)")
        }
    }
}